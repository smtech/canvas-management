...
$turnAroundComparison = buildPageSection('
	...
	<div class="image-placement" style="float: right; width: ' . GRAPH_INSET_WIDTH . ';">
		...
		<a
			...
			href="graph/turn-around-comparison.php?course_id=' . $_REQUEST['course_id'] . '&department_id=' . $statistic['course[account_id]'] . '"
		>
			<img src="graph/turn-around-comparison.php?course_id=' . $_REQUEST['course_id'] . '&department_id=' . $statistic['course[account_id]'] . '"  style="width: 100%;" />
	...
	<p class="caption">The average turn-around time across all courses (<span class="average-underline">' . GRAPH_AVERAGE_STYLE . ' ' . GRAPH_AVERAGE_COLOR . ' line</span> above), weighted by number of graded assignments and students, is ' . round(averageTurnAround(), 1) . ' days. The average turn-around time across ' . $statistic['account[name]'] . ' courses (<span class="average-underline">' . GRAPH_AVERAGE_STYLE . ' ' . GRAPH_AVERAGE_COLOR . ' line</span> at right) is ' . round(averageTurnAround($statistic['course[account_id]']), 1) . ' days. In &ldquo;' . $statistic['course[name]'] . '&rdquo; (<span class="highlight-column"></span>' . GRAPH_HIGHLIGHT_COLOR . ' column), the current average is ' . round($statistic['average_grading_turn_around'], 1) . ' days.</p>
...
$assignmentCount = buildPageSection('
	...
	<div class="image-placement" style="width: ' . GRAPH_INSET_WIDTH . '; float: right;">
		...
		<a
			...
			href="graph/assignment-count-comparison.php?course_id=' . $_REQUEST['course_id'] . '&department_id=' . $statistic['course[account_id]'] . '"
		>
			<img src="graph/assignment-count-comparison.php?course_id=' . $_REQUEST['course_id'] . '&department_id=' . $statistic['course[account_id]'] . '" width="100%" />
			<p class="caption">&ldquo;' . $statistic['course[name]'] . '&rdquo; compared to all active courses in the ' . $statistic['account[name]'] . ' department.</p>
		</a>
	</div>
	...
	<p class="caption">The average number of assignments posted to Canvas (<span class="average-underline">' . GRAPH_AVERAGE_STYLE . ' ' . GRAPH_AVERAGE_COLOR .' line</span> above) across all courses is ' . round(averageAssignmentCount(), 1) . '. The average number of assignments posted to Canvas (<span class="average-underline">' . GRAPH_AVERAGE_STYLE . ' ' . GRAPH_AVERAGE_COLOR . ' line</span> at right) in the ' . $statistic['account[name]'] . ' department is ' . round(averageAssignmentCount($statistic['course[account_id]']), 1) . '. In &ldquo;' . $statistic['course[name]'] . '&rdquo; (<span class="highlight-column"></span>' . GRAPH_HIGHLIGHT_COLOR . ' column), there are ' . ($statistic['assignments_due_count'] + $statistic['dateless_assignment_count']) . ' assignments posted to Canvas.</p>
...
if (strlen($statistic['oldest_ungraded_assignment_url'])) {	
	...
	$numbers .= '
		...
		<div class="image-placement">
			<p class="caption"><a style="border-bottom: dotted 1px black;" target="_blank" href="' . $statistic['oldest_ungraded_assignment_url'] . '">' . $statistic['oldest_ungraded_assignment_name'] . '</a>, due ' . $oldestDueDate->format('l, F j, Y') . ' was due ' . $now->diff($oldestDueDate)->format('%a') . ' days ago and no submissions have been graded.</p>
		</div>
...
if (round($statistic['average_submissions_graded']*100, 0) < 100) {
	$numbers .= '
		...
		<div class="image-placement">
			<p class="caption">For each assignment in &ldquo;' . $statistic['course[name]'] . '&rdquo;, on average ' . round($statistic['average_submissions_graded']*100, 0) . '% of the student submissions have been graded.</p>
		</div>
